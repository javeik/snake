...
export class Canvas extends React.Component<{}, ICanvasState> {
    ...
    private handleClicking(event: React.MouseEvent<HTMLCanvasElement>) {        
        this.setState({ targetX: event.clientX - this.drawingCanvas!.getBoundingClientRect().left, targetY: event.clientY - this.drawingCanvas!.getBoundingClientRect().top });
    ...
    private calculateTrajectory() {
        ...
        this.snake.setSpeed(dX < 0 ? this.baseSpeed * -1 : this.baseSpeed,
    ...
    private checkIfArrivedAtTarget() {
        if (this.snake.x === this.state.targetX &&
    ...
    public render() {
        return (
            <div>
                <Header />
                <div>
                    <canvas ref={(canvas) => { this.drawingCanvas = canvas; }} className="drawingCanvas-canvas"
                        width={this.canvasWidth} height={this.canvasHeight} onClick={event => this.handleClicking(event)} />
                </div>
                <div>
                    <DebugPanel x={this.snake.x} y={this.snake.y} rectLength={this.canvasHeight} rectWidth={this.canvasWidth}
                        speedX={this.snake.getSpeed().xSpeed} speedY={this.snake.getSpeed().ySpeed} clickX={this.state.targetX} clickY={this.state.targetY}
